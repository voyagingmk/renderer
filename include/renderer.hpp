...
namespace renderer {
	...
	public:
		...
		PtrColor rayTraceRecursive(PtShape scene, PtrRay ray, int maxReflect);
		void rayTraceReflection(cil::CImg<unsigned char> &img, PtShape scene, PerspectiveCamera& camera, int maxReflect, int x = 0, int y = 0, int w = 0, int h = 0);
	};
}
