...
int Parser::parseFromFile(std::string path){
	...
	if(!multithread){
		...
		if (multithread == 2) {
			std::thread t1(renderArea, std::ref(renderer), std::ref(img), std::static_pointer_cast<Geometry>(pUnion), std::ref(camera), 4, 0, 0, width, height / 4);
			std::thread t2(renderArea, std::ref(renderer), std::ref(img), std::static_pointer_cast<Geometry>(pUnion), std::ref(camera), 4, 0, 1 * height / 4, width, height / 4);
			std::thread t3(renderArea, std::ref(renderer), std::ref(img), std::static_pointer_cast<Geometry>(pUnion), std::ref(camera), 4, 0, 2 * height / 4, width, height / 4);
			std::thread t4(renderArea, std::ref(renderer), std::ref(img), std::static_pointer_cast<Geometry>(pUnion), std::ref(camera), 4, 0, 3 * height / 4, width, height / 4);
			t1.join();
			t2.join();
			t3.join();
			t4.join();
		...
		if (multithread == 3) {
			std::thread t1(renderArea, std::ref(renderer), std::ref(img), std::static_pointer_cast<Geometry>(pUnion), std::ref(camera), 4, 0, 0, width, height / 8);
			std::thread t2(renderArea, std::ref(renderer), std::ref(img), std::static_pointer_cast<Geometry>(pUnion), std::ref(camera), 4, 0, 1 * height / 8, width, height / 8);
			std::thread t3(renderArea, std::ref(renderer), std::ref(img), std::static_pointer_cast<Geometry>(pUnion), std::ref(camera), 4, 0, 2 * height / 8, width, height / 8);
			std::thread t4(renderArea, std::ref(renderer), std::ref(img), std::static_pointer_cast<Geometry>(pUnion), std::ref(camera), 4, 0, 3 * height / 8, width, height / 8);
			std::thread t5(renderArea, std::ref(renderer), std::ref(img), std::static_pointer_cast<Geometry>(pUnion), std::ref(camera), 4, 0, 4 * height / 8, width, height / 8);
			std::thread t6(renderArea, std::ref(renderer), std::ref(img), std::static_pointer_cast<Geometry>(pUnion), std::ref(camera), 4, 0, 5 * height / 8, width, height / 8);
			std::thread t7(renderArea, std::ref(renderer), std::ref(img), std::static_pointer_cast<Geometry>(pUnion), std::ref(camera), 4, 0, 6 * height / 8, width, height / 8);
			std::thread t8(renderArea, std::ref(renderer), std::ref(img), std::static_pointer_cast<Geometry>(pUnion), std::ref(camera), 4, 0, 7 * height / 8, width, height / 8);
			t1.join();
			t2.join();
			t3.join();
			t4.join();
